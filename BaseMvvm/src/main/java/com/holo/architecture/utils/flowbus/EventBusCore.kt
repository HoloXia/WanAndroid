...
import com.holo.architecture.ext.logW
...
class EventBusCore : ViewModel() {
    ...
    fun postEvent(eventName: String, value: Any, timeMillis: Long) {
        "post Event:$eventName".logW("FlowBus")
        listOfNotNull(
    ...
    private fun <T : Any> invokeReceived(value: Any, onReceived: (T) -> Unit) {
        try {
            ...
            KLog.e("FlowBus", "class cast error on message received: $value", e)
        } catch (e: Exception) {
            KLog.e("FlowBus", "error on message received: $value", e)
        }
    }
